...
@click.command()
@click.option('--exp', type=click.Choice(['svhn_mnist', 'svhn_mnist_rgb']), default='svhn_mnist', help='experiment to run')
...
@click.option('--src_intens_flip', is_flag=True, default=False, help='src aug colour; enable intensity flip')
...
@click.option('--seed', type=int, default=3101999, help='random seed (0 for time-based)')
...
def experiment(exp, arch, loss, double_softmax, confidence_thresh, rampup, teacher_alpha, fix_ema,
    ...
    writer = SummaryWriter(log_dir='runs/{}-{}-{}'.format(exp, str(num_epochs), str(batch_size) ) )

    if log_file == '':
        log_file = 'txt_logs/output_log_{}-{}-{}.txt'.format(exp,str(num_epochs),str(batch_size) )
    elif log_file == 'none':
        log_file = None
    ...
    def f_train(X_src, y_src, X_tgt0, X_tgt1):
        ...
        # writer.add_embedding( teacher_tgt_logits_out, label_img=X_tgt1, global_step=1, tag='idk')
    ...
    for epoch in range(num_epochs):
        ...
        tsne_num = 40
        mytestx = torch.tensor(
            d_target.test_X[:tsne_num], dtype=torch.float, device=torch_device)
        mytesty = d_target.test_y[:tsne_num]
        out = student_net(mytestx)
        writer.add_embedding( out, label_img=mytestx, metadata=mytesty, global_step=epoch+1, tag="epoch"+str(epoch))
        writer.flush()
        ...
        if conf_mask_rate > best_conf_mask_rate:
            ...
            improve = ''
        unsup_loss_string = '{}, conf mask={:.3%}, unsup mask={:.3%}'.format(unsup_loss_string, conf_mask_rate, unsup_mask_rate)

        t2 = time.time()

        writer.add_scalar('Error/src_test_err_stu', src_test_err_stu, epoch+1)
        writer.add_scalar('Error/tgt_test_err_stu', tgt_test_err_stu, epoch+1)
        writer.add_scalar('Error/tgt_test_err_tea', tgt_test_err_tea, epoch+1)
